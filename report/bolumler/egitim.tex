...
\textbf{Görev sınıfsız tek nesne takibi.} Operatör ekranda hedefi bir kutuyla
işaretliyor, model o nesneyi video boyunca takip ediyor. Hiçbir yerde sınıf
etiketi sorulmuyor: ``bu bir araç mı'' değil, ``işaretlediğim şey şimdi
nerede''. Bu tanım aşağıdaki birkaç kararı doğrudan belirliyor, en çok da
teacher seçimini.
...
\textbf{Somut olarak koşulan şey} tek bir notebook ve altı adımdan ibaret:

\begin{figure}[H]
  \centering
  \begin{tikzpicture}[font=\elyazi\small]
    \eskizbasla[73]
    % --- üst sıra: ucuz olan, önce ---
    \node[esbaslik, anchor=east, text width=20mm] at (0.3,0.8)
      {\textbf{önce ucuz}\\{\footnotesize olan}};
    \node[kgri,  text width=30mm] (a1) at (2.5,0.8)
      {\textbf{1} veriyi indir\\{\footnotesize iki modaliteyi\\eşleştir}};
    \node[ksari, text width=30mm] (a2) at (7.0,0.8)
      {\textbf{2} kaynaşmayı say\\{\footnotesize GPU'ya dokunmadan}};
    \node[ksari, text width=30mm] (a3) at (11.5,0.8)
      {\textbf{3} tek window'a\\overfit\\{\footnotesize kurulum doğru mu}};

    \draw[esok] (a1) -- (a2);
    \draw[esok] (a2) -- (a3);

    % --- alt sıra: pahalı olan ---
    \node[esbaslik, anchor=east, text width=20mm] at (0.3,-2.6)
      {\textbf{sonra}\\{\footnotesize pahalı olan}};
    \node[kmavi,  text width=30mm] (b1) at (2.5,-2.6)
      {\textbf{4} aşama A\\{\footnotesize distilasyon}};
    \node[kmavi,  text width=30mm] (b2) at (7.0,-2.6)
      {\textbf{5} aşama B, $\times$\,\textbf{3}\\
       {\footnotesize stok / A'dan / LoRA}};
    \node[kyesil, text width=30mm] (b3) at (11.5,-2.6)
      {\textbf{6} üçünü aynı split'te skorla};

    \draw[esok] (b1) -- (b2);
    \draw[esok] (b2) -- (b3);

    % üst sıradan alt sıraya geçiş: ortadan aşağı, yani "önce bu sıra,
    % sonra bu sıra" -- tek bir kutudan tek bir kutuya değil
    \draw[esok] (a2.south) -- (b2.north);

    \node[esvurgu, anchor=west, text width=46mm] at (7.4,-0.9)
      {2 kötü çıkarsa 4 ve 5 boşa gider};
  \end{tikzpicture}
  \caption{Notebook'un adımları. Sıra keyfi değil, ucuzdan pahalıya:
    GPU istemeyen ya da dakikalar süren kontroller önde duruyor, çünkü
    onlardan biri kötü çıkarsa arkasındaki GPU saatlerinin hepsi bozuk bir
    hedefin üstüne harcanmış olur.}
  \label{fig:egitim-notebook}
\end{figure}

Bölümün geri kalanı bu adımların her birinin neden öyle olduğunu anlatıyor.
...
Üç aday var. En görünür fark grid'lerinde, yani teacher'ın feature
map'inin loss'tan önce resample edilip edilmediğinde; seçimi belirleyen ise
başka bir şey.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[font=\elyazi\small]
    \eskizbasla[19]
    \node[kmor, text width=34mm] (s2) at (0,0)
      {\textbf{SAM 2.1} Hiera-B+\\{\footnotesize $1024/16 = 64{\times}64$}\\
       {\footnotesize \emph{varsayılan}}};
    \node[kgri, text width=34mm] (d3) at (5.3,0)
      {DINOv3 ViT-B/16\\{\footnotesize $512/16 = 32{\times}32$}\\
       {\footnotesize ikinci koşu}};
    \node[kgri, text width=34mm] (d2) at (10.6,0)
    ...
    \draw[esokr=esmor]   (0,-1.15) -- (0,-4.0) -- (g.west);
    \draw[esokr=esyesil] (d3) -- (g);
    \draw[esink] (10.6,-1.15) -- (10.6,-4.0) -- (g.east);

    % üç etiket, üçü de kendi çizgisinin farklı yanında: aynı yükseklikte
    % oldukları için aksi hâlde üst üste binerler
    \node[esnot, anchor=west, xshift=1.5mm, text width=26mm] at (0,-2.4)
      {tam \textbf{2 kat}\\küçültme};
    \node[esiyi, anchor=east, xshift=-1.5mm, text width=20mm] at (5.3,-2.4)
      {\textbf{birebir}};
    \node[esnot, anchor=east, xshift=-1.5mm, text width=26mm] at (10.6,-2.4)
      {kesirli\\interpolasyon};
  \end{tikzpicture}
  \caption{Teacher adaylarının grid'i, student'ınkiyle karşılaştırmalı.
    ...
    veriyor; SAM 2.1'in $64\times64$'ü tam iki kat, o da temiz; yalnızca
    DINOv2'nin 37'si kesirli bir orana düşüyor. Varsayılanı belirleyen şey
    grid değil, bir sonraki tablo.}
  \label{fig:egitim-ogretmen}
\end{figure}

Üçü de kodda hazır ve seçim tek bir string. Asıl gerilim DINOv3 ile
SAM 2.1 arasında, ve şöyle duruyor:
...
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{26mm}
                  ...
                  >{\raggedright\arraybackslash}p{54mm}@{}}
  \toprule
   & \textbf{DINOv3} ViT-B/16 & \textbf{SAM 2.1} Hiera-B+ \\
  \midrule
  feature'lar ne taşıyor
    & anlam: ``bu bir araç''
    & sınır ve nesnelik: ``burada bir şey var, şurada bitiyor'' \\[1.2mm]
  göreve uyumu
    & sınıf sormuyoruz, yani anlam karşılığı olmayan bir fazlalık
    & görev tanımının kendisi: sınıfsız tek nesne \\[1.2mm]
  tensör uyumu
    & 768 kanal, projection bir temsil değişikliği
    & 256 kanal ve stride 16, student'ınkiyle birebir \\[1.2mm]
  checkpoint'e uyumu
    & ilgisiz bir uzay
    & EdgeTAM zaten SAM~2'nin distilasyonu \\[1.2mm]
  hiza riski
    & encoder'ı memory yolunun beklediği uzaydan uzaklaştırıyor
    & aynı uzaya doğru itiyor, yani riski küçültüyor \\[1.2mm]
  grid
    & $32\times32$, resample yok
    & $64\times64$, tam iki kat küçültme \\[1.2mm]
  adım maliyeti
    & 512 girdide ucuz
    & 1024 girdide yaklaşık üç katı \\
  \bottomrule
\end{tabular}
\end{center}

\textbf{Varsayılan SAM 2.1 Hiera-B+.} İlk iki satır tek başına belirleyici.
Görev hiçbir yerde sınıf sormuyor, dolayısıyla DINOv3'ün asıl gücü olan
anlam burada karşılığı olmayan bir fazlalık; SAM 2'nin ``ne olduğunu
bilmeden nerede bittiğini bilme'' hâli ise görevin tanımının kendisi.
Alttaki satırlar da aynı yöne bakıyor.

Large değil B+ olmasının sebebi hesap değil. EdgeTAM'ın kendi makalesi
teacher olarak SAM~2 Hiera-B+'yı almış ve hedef olarak stride 16'daki
encoder feature map'ini kullanmış~\cite{zhou2025edgetam}, yani bizim
okuduğumuz tensörün aynısını. EdgeTAM'ın gövde ağırlıkları \emph{zaten} o
hedefe oturmuş ağırlıklar. B+'dan distile etmek o hedefi kaldığı yerden
sürdürmek oluyor, üstüne yalnızca modalite boşluğu eklenmiş hâlde;
Large'dan distile etmek ise student'ı başlangıç noktasının hedefinden
\emph{başka} bir hedefe çekmek olurdu, ki bu SAM teacher'ını ilginç kılan
tek özelliği feda etmek demek.

\textbf{DINOv3 ikinci koşu olarak duruyor.} Üst sınırı daha yüksek olabilir,
ama hiza riskini o taşıyor. Aradaki tek fark bir string; veri, seed,
schedule ve loss aynı kalıyor, dolayısıyla çıkan fark teacher'ın kendisine
ait oluyor.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[font=\elyazi\small]
    ...
    \node[kmavi,  text width=32mm] at (4.5,-1.9)
      {{\footnotesize \textbf{2 864} çift}\\
       {\footnotesize \textcolor{eskirmizi}{1 160 bozuk kare atıldı}}};
...
oluyor. SegFly hacmi getiriyor (temizlenmiş Kust4K'nın beş katı kare, üç irtifa),
...
Bir de \textbf{bozuk kare} ayıklaması var. Kust4K'nın 4 024 karesinin
1 160'ında, yani yüzde 29'unda, iki modaliteden biri sensör arızasını
taklit etmek için kasten bozulmuş. Görüntüler sorunsuz açılıyor, sadece
sahnenin kendisi değiller, yani gözle bakmayan bir okuyucu için görünmez
bir gürültü. Set bu kareleri kendi manifest dosyalarında adlarıyla
sayıyor; okuyucu o listeleri okuyup kareleri atıyor, geriye 2 864 temiz
çift kalıyor.
...
Sebebi geometrik: hedefler karenin yüzde biri kadar, yani onlarca piksel
genişliğinde, ve birkaç piksellik sistematik bir kayma o boyutta maskenin
önemli bir kısmını kaydırıyor. Aynı kayma büyük bir hedefte fark
edilmiyor bile, yani hasar hedef boyutuyla ters orantılı ve en çok bizim
en çok baktığımız boyutta canını yakıyor. VTUAV'ın kendi belgesi de iki
modalitenin piksel piksel hizalı \emph{olmadığını} söylüyor. O yüzden fikir
setten sete ayrı ayrı ölçülmeden uygulanmıyor, ve ölçüm ucuz: her set için
bir kayma tahmini çıkarıp taşınan maskenin kendi ground truth'uyla IoU'suna
bakmak yetiyor.
