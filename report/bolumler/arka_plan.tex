...
bir tanıtımı değil, EdgeTAM'ı hızlı kılan tasarım kararının, yani
memory attention'ın token sayısını küçültmenin, somut sayılarla
...
\begin{itemize}
  \item \textbf{memory bank}: son birkaç karenin (bu projede
        kullanılan checkpoint'te en fazla 7, yani
        \texttt{num\_maskmem}) özellik haritalarını ve object
        pointer'larını tutan, kare indeksiyle anahtarlanmış bir yapı;
...
\begin{figure}[H]
  \centering
  \gorselyertutucu{SAM 2 makalesinden~\cite{ravi2024sam2} alınan orijinal
    mimari şeması: image encoder, memory bank, memory attention, memory
    encoder ve mask decoder arasındaki veri akışı}
  \caption{SAM 2'nin orijinal mimarisi.}
  \label{fig:sam2-mimari-orijinal}
\end{figure}

\subsection{memory attention'ın gerçek maliyeti}

SAM 2'nin checkpoint'i 1024×1024 girdiyle çalışır; image encoder'ın
...
değil, \textbf{on altı} katına çıkarır, çünkü hem sorgu hem anahtar
tarafı karesel olarak büyür. Bu, SAM 2'nin edge cihazlarda neden
...
\subsection{Ölçülen Maliyet Dağılımı}
\label{sec:proje-eslesme}

Yukarıdaki karmaşıklık analizi teoriktir; bu çalışma bunu
\texttt{tools/analyze\_cost.py} ile gerçek checkpoint üzerinde
doğrulamıştır: EdgeTAM checkpoint'inin dört hesaplama bloğunun,
1024×1024 çözünürlükte, tek nesneyle ölçülen işlem yükü (FLOP) ve
parametre sayıları böyledir:
...
Bu ölçüm, çalışmanın kapsamını tek bir modülden EdgeTAM'ın
\textbf{dört} ana hesaplama bloğunun tamamına genişletmiştir -- ve tam
olarak bu maliyet profilini hedef alan mimari, sıradaki bölümün
konusudur.
...
Bu sıkıştırmanın çalışma biçimi cross-attention'ın temel bir özelliğine
dayanır: çıktının token sayısı anahtar/değer (key/value) tarafından
değil sorgu (query) tarafından belirlenir. Perceiver'ın $N_g{+}N_l$
sorgusu karenin kendisinden gelmez; eğitim sırasında güncellenen,
veriden bağımsız sabit bir ağırlık matrisidir. Bu sorgular hafıza
karesinin 4096 token'ına bakıp özet çıkarırken çıktı her zaman sorgu
sayısı kadardır: kare 4096 token'lık kalsa da sıkıştırılmış temsil hep
512 token'dır.

Yalnızca global latent'lar yeterli değildir; 2D spatial latent'ların
ayrıca eklenmesinin nedeni budur. EdgeTAM makalesi~\cite{zhou2025edgetam},
segmentasyonun \emph{yoğun} (dense) bir tahmin görevi olduğunu ve
memory bank'ın uzamsal yapısının korunması gerektiğini, oysa naif bir
Perceiver'ın bu yapıyı attığını belirtir. Makalenin kendi ablation'ı
(SA-V val) bunu ölçer: yalnızca 256 global latent \textbf{62,0} J\&F
verirken, 256 global + 256 2D spatial latent \textbf{64,4} J\&F'e
çıkar. Bu \textbf{2,4} puanlık fark, 2D spatial latent'ların süs değil
gereklilik olduğunu gösterir: mask decoder'ın maskeyi hangi bölgede
çizeceğini bilmesi gerekir, ki bu bilgi salt global özetlerde yoktur.
...
davranışını taklit etme. Görüntü aşamasında image encoder çıktıları
arasına, video aşamasında ayrıca memory attention çıktıları arasına bir
hizalama (MSE) kaybı eklenir; öğrenci yalnızca nihai maskeyi değil
öğretmenin ara temsillerini de taklit etmeye zorlanır. EdgeTAM
makalesi~\cite{zhou2025edgetam} bu boru hattının SA-V val'de
\textbf{+1,3}, SA-V test'te \textbf{+3,3} puan J\&F kazandırdığını
raporlar. Bu, EdgeTAM'ın küçük bir backbone (RepViT tabanlı) ve
sıkıştırılmış bir hafıza ile SAM 2'ye yakın doğruluğa ulaşmasını
sağlayan eğitim tarafındaki tamamlayıcı unsurdur; bu raporun konusu
olan inference hızlandırması bu eğitimden sonraki, sabit ağırlıklı
checkpoint üzerinde çalışır.

\begin{figure}[H]
  \centering
  \gorselyertutucu{EdgeTAM makalesinden~\cite{zhou2025edgetam} alınan
    knowledge distillation şeması: öğretmen/öğrenci paralel yolları,
    image encoder ve memory attention çıktılarında hizalama kaybı}
  \caption{EdgeTAM'ın knowledge distillation boru hattı.}
  \label{fig:edgetam-kd}
\end{figure}
