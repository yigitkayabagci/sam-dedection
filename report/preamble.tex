...
% Uzun \texttt{} tanımlayıcıları (dosya yolları, fonksiyon adları) satır
% sonunda bölünemez ve satırı marja taşırırdı. \emergencystretch, TeX'e
% yalnızca taşacak olan paragraflarda kelime arasını biraz esnetme izni
% verir; normal paragraflar etkilenmez.
...
% El çizimi ("Excalidraw") şekil stilleri -- yalnızca encoder eğitimi
% bölümü kullanıyor, ama palet ve yazı tipi tanımları burada yükleniyor
% çünkü aşağıdaki \newtcolorbox{tasarim} eskiz paletinden renk alıyor.
\input{eskiz}

% normalem: ulem varsayılan olarak \emph'i altı çizili yapıyor, ki raporun
% geri kalanı \emph'i italik olarak kullanıyor. Buradan tek istenen \sout
% (üstü çizili) -- eğitim bölümünde "bu terim hiç yok" demenin görsel yolu.
...
% Kodu yazılmış ama henüz koşulmamış bir tasarım kararı. Encoder eğitimi
% bölümünün tamamı bu kategoride: orada anlatılan her şey depoda çalışır
% hâlde duruyor, hiçbiri bir GPU koşusuyla ölçülmedi. Yukarıdaki üç kutudan
% ayrı bir kutu olmasının sebebi de bu -- "ölçülmedi" ile "ölçülecek" aynı
% şey değil, ve bir tasarım gerekçesi bir sayı yerine geçmez.
\newtcolorbox{tasarim}[1][]{
  enhanced, breakable, colback=esmord!22, colframe=esmor,
  boxrule=0.6pt, left=2mm, right=2mm, top=1mm, bottom=1mm,
  fonttitle=\sffamily\bfseries, coltitle=white,
  colbacktitle=esmor, title={Tasarım kararı: kod yazıldı, ölçüm bekliyor}, #1,
}
...
% Eksik görsel koruması: figures/ klasöründeki PNG'ler depoya
% işlenmiyor (yerel görseller), bu yüzden dosyaya bağlı bir
% \includegraphics çağrısı başka bir makinede derlemeyi kırardı.
% Aşağıdaki sarmalayıcı, dosya varsa gerçek görseli çizer; yoksa
% \gorselyertutucu kutusuna düşer. Görseller yerinde olan makinede
% davranış hiç değişmez.
\let\gercekincludegraphics\includegraphics
\renewcommand{\includegraphics}[2][width=\linewidth]{%
  \IfFileExists{#2}%
    {\gercekincludegraphics[#1]{#2}}%
    {\gorselyertutucu{eksik dosya: {\scriptsize\texttt{\detokenize{#2}}}}}%
}

% Encoder eğitimi bölümü iki yerde derleniyor: ana raporun içinde
% (main.tex) ve tek başına (bolum6.tex). Tek başınayken diğer bölümler
% ortada yok, dolayısıyla onlara yapılan \cref'ler çözülemez. \diger iki
% tam ifade alıyor: birincisi ana raporda, ikincisi ayrı basımda yazılan.
% Ekler de ifadenin içinde ("...'de" ile "...bölümünde" aynı şey değil).
% bolum6.tex kendi tanımını \input{preamble}'dan *önce* yapıyor, o yüzden
% burası \providecommand.
\providecommand{\diger}[2]{#1}

% Tabloda havadan (UAV) çekim yapan veri setlerini işaretleyen yıldız.
\newcommand{\hava}{\textsuperscript{$\star$}}

% Kendi çekimi olmayıp başka veri setlerinden türetilmiş setleri
% işaretleyen hançer (bkz. veri_setleri.tex, "Ortak Kaynaklar").
\newcommand{\turetilmis}{\textsuperscript{$\dagger$}}
...
% ---- İçindekiler: tek sayfaya sığması için sıkıştırılmış aralık -------------
% Encoder eğitimi bölümü on bir kısım getirince liste iki sayfaya taşmaya
% başladı. İki ayarla geri sığdırıldı: bölüm öncesi aralık 0,3em'den
% 0,15em'e, ve main.tex'te \tableofcontents'in etrafındaki \linespread.
% Yeni bir bölüm eklenirse ilk bakılacak yer bu ikisi.
\DeclareTOCStyleEntry[beforeskip=0.15em plus 1pt]{tocline}{chapter}
