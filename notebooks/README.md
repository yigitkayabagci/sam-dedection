...
| 19 | `19_kaggle_uav_thermal_pool.ipynb` | **`llvip` alone** out of the Kaggle merged UAV-thermal set (27 925 images, 6 sources, CC0) — its 15 488 infrared frames are the only reason the 13 GB download is worth making, and they are still cut **to target scale** before a single image is written to disk: labels indexed out of the archive, close-ups dropped on the median box, near targets capped at a quota so the model still knows they exist, then SAM 3 over what survives. The other five sources are named and argued in the generator; the modality probe still measures all six | Kaggle API credentials (`KAGGLE_USERNAME` / `KAGGLE_KEY` Colab secrets) and an HF token for SAM 3 |
| 20 | `20_aerovis_stage_b_pool.ipynb` | AeroVIS translated into a stage-B pool with **no teacher at all** — it already ships 1 378 603 (image, box, mask) triples, so this converts YTVIS RLE into the same store `image_loop.py` reads, on a CPU runtime. Splits whole sequences stratified by source, because vehicle is UAVDT-only and boat SeaDronesSee-only | nothing — it fetches the 12.6 GiB archive itself. `HOLDOUT` decides whether AeroVIS stays a held-out benchmark |
