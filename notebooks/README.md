...
Fifteen notebooks, meant for Colab. Everything they orchestrate lives in
...
| 15 | `15_semantic_mask_pool.ipynb` | **instances out of semantic maps** — a click separates what `decompose` fuses, the drawn map verifies it, and iSAID's drawn instances say first whether it beat the baseline | **nothing** — Kust4K and iSAID's annotations are fetched in the notebook |
...
**15 is the third labeller, and the one that needs no boxes.** 13 and 14 turn
a human's box into a mask; 15 turns a human's *semantic map* into **instances**
— the thing `aerial.decompose` has been approximating and getting wrong in one
specific way. Measured: SegFly one-car-one-blob recovery **78.5 %**, iSAID
**82.0 %**, and a real six-vehicle blob passes all four `InstanceGates` at
fill 0.79. The map cannot separate two cars parked flush because the boundary
between them is not a class boundary; the image still has it. So the component
becomes a *region*, the number of objects in it is estimated from the
dataset's own median, that many interior points are seeded, and each click
gets segmented separately — with the drawn map then checking, pixel by pixel,
that what came back is the right class (`purity`, the gate a box pool cannot
have). Generated by `tools/build_semantic_notebook.py`; machinery in
`src/training/semantic.py`; argued in `docs/semantic_rescue_plan.md`. Its
first real output is a **measurement**: both routes run over iSAID's drawn
instances flattened to the map they imply, and if the rescue does not beat
plain `decompose` on recall there, the notebook says to stop.
